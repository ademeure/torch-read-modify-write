...
def _squared_relu_bwd_kernel(
    ...
    GRAD_IS_BF16: tl.constexpr, BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    grad_raw = tl.load(grad_ptr + offs, mask=mask)
    # Cast grad to fp32 if input is bf16
    grad = grad_raw.to(tl.float32) if GRAD_IS_BF16 else grad_raw
...
def triton_squared_relu_bwd(grad, relu_fp32, relu_bf16):
    """Fused squared relu backward: grad * 2 * relu * (relu > 0) → bf16. Accepts fp32 or bf16 grad."""
    n = grad.numel()
    out = torch.empty(grad.shape, dtype=torch.bfloat16, device=grad.device)
    BLOCK = 1024
    grid = ((n + BLOCK - 1) // BLOCK,)
    is_bf16 = (grad.dtype == torch.bfloat16)
    _squared_relu_bwd_kernel[grid](grad, relu_fp32, relu_bf16, out, n, GRAD_IS_BF16=is_bf16, BLOCK=BLOCK)
    return out
...
# ── Triton fused squared relu forward kernel ────────────────────────
@triton.jit
def _squared_relu_fwd_kernel(
    ...
    mask = offs < n
    x = tl.load(x_ptr + offs, mask=mask)
    relu_bf16 = tl.where(x > 0, x, tl.zeros_like(x))
    tl.store(relu_bf16_ptr + offs, relu_bf16, mask=mask)
    relu_fp32 = relu_bf16.to(tl.float32)
    tl.store(relu_fp32_ptr + offs, relu_fp32, mask=mask)
    sq = relu_fp32 * relu_fp32
    tl.store(sq_bf16_ptr + offs, sq.to(tl.bfloat16), mask=mask)

def triton_squared_relu_fwd(x_bf16):
    """Fused relu + to_fp32 + square + to_bf16."""
    n = x_bf16.numel()
    shape = x_bf16.shape
    relu_bf16 = torch.empty(shape, dtype=torch.bfloat16, device=x_bf16.device)
    relu_fp32 = torch.empty(shape, dtype=torch.float32, device=x_bf16.device)
    sq_bf16 = torch.empty(shape, dtype=torch.bfloat16, device=x_bf16.device)
    BLOCK = 1024
    grid = ((n + BLOCK - 1) // BLOCK,)
    _squared_relu_fwd_kernel[grid](x_bf16, relu_bf16, relu_fp32, sq_bf16, n, BLOCK=BLOCK)
    return relu_bf16, relu_fp32, sq_bf16
# ── End Triton fused squared relu forward kernel ──────────────────
...
def forward(
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h0_mlp_detach, h0_mlp__to_copy, _sq_bf16_0 = triton_squared_relu_fwd(h0_mlp_c_fc__unsafe_view)
    ...
    h0_mlp_c_proj__to_copy_1 = _sq_bf16_0  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h1_mlp_detach, h1_mlp__to_copy, _sq_bf16_1 = triton_squared_relu_fwd(h1_mlp_c_fc__unsafe_view)
    ...
    h1_mlp_c_proj__to_copy_1 = _sq_bf16_1  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h2_mlp_detach, h2_mlp__to_copy, _sq_bf16_2 = triton_squared_relu_fwd(h2_mlp_c_fc__unsafe_view)
    ...
    h2_mlp_c_proj__to_copy_1 = _sq_bf16_2  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h3_mlp_detach, h3_mlp__to_copy, _sq_bf16_3 = triton_squared_relu_fwd(h3_mlp_c_fc__unsafe_view)
    ...
    h3_mlp_c_proj__to_copy_1 = _sq_bf16_3  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h4_mlp_detach, h4_mlp__to_copy, _sq_bf16_4 = triton_squared_relu_fwd(h4_mlp_c_fc__unsafe_view)
    ...
    h4_mlp_c_proj__to_copy_1 = _sq_bf16_4  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h5_mlp_detach, h5_mlp__to_copy, _sq_bf16_5 = triton_squared_relu_fwd(h5_mlp_c_fc__unsafe_view)
    ...
    h5_mlp_c_proj__to_copy_1 = _sq_bf16_5  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h6_mlp_detach, h6_mlp__to_copy, _sq_bf16_6 = triton_squared_relu_fwd(h6_mlp_c_fc__unsafe_view)
    ...
    h6_mlp_c_proj__to_copy_1 = _sq_bf16_6  # from fused squared relu
    ...
    # x = F.relu(x).square()
    # FUSED: relu + to_fp32 + square + to_bf16 in single Triton kernel
    h7_mlp_detach, h7_mlp__to_copy, _sq_bf16_7 = triton_squared_relu_fwd(h7_mlp_c_fc__unsafe_view)
    ...
    h7_mlp_c_proj__to_copy_1 = _sq_bf16_7  # from fused squared relu
...
def backward(
    ...
    grad_h7_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h7_mlp_c_proj_view_1, _to_copy_65, detach_52)
    ...
    # FUSED: backward RoPE for K (inverse rotation via triton_rope_fwd with -sin)
    _bwd_neg_sin = aten.neg(slice_2)
    add_69: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h7_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q (inverse rotation via triton_rope_fwd with -sin)
    add_72: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h7_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    grad_h6_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h6_mlp_c_proj_view_1, _to_copy_56, detach_45)
    ...
    grad_h6_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h6_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_80: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h6_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_83: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h6_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h5_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h5_mlp_c_proj_view_1, _to_copy_48, detach_39)
    ...
    grad_h5_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h5_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_93: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h5_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_96: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h5_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h4_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h4_mlp_c_proj_view_1, _to_copy_39, detach_32)
    ...
    grad_h4_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h4_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_107: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h4_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_110: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h4_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h3_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h3_mlp_c_proj_view_1, _to_copy_31, detach_26)
    ...
    grad_h3_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h3_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_120: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h3_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_123: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h3_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h2_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h2_mlp_c_proj_view_1, _to_copy_22, detach_19)
    ...
    grad_h2_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h2_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_134: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h2_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_137: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h2_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h1_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h1_mlp_c_proj_view_1, _to_copy_14, detach_13)
    ...
    grad_h1_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h1_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_147: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h1_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_150: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h1_attn_getitem_4, slice_1, _bwd_neg_sin)
    ...
    # x = F.relu(x).square()
    # FUSED: squared_relu backward
    grad_h0_mlp_threshold_backward: 'bfloat16[32, 2048, 2048]' = triton_squared_relu_bwd(grad_h0_mlp_c_proj_view_1, _to_copy_5, detach_6)
    ...
    grad_h0_attn_getitem_4: 'bfloat16[32, 2048, 4, 128]' = operator.getitem(grad_h0_attn__fused_rms_norm_backward_1, 0)  # strides=(1048576, 512, 128, 1), contiguous=True, view=True
        # FUSED: backward RoPE for K
    _bwd_neg_sin = aten.neg(slice_2)
    add_161: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h0_attn_getitem_3, slice_1, _bwd_neg_sin)
    # FUSED: backward RoPE for Q
    add_164: 'bfloat16[32, 2048, 4, 128]' = triton_rope_fwd(grad_h0_attn_getitem_4, slice_1, _bwd_neg_sin)
