"""016_model.py:49 [blocks.0] | x = x + self.mlp(self.ln_2(x))

Inputs (36.9KB total):
  add_1       float32[2x16x32]  4.0KB
  primals_11  float32[32]  128B
  primals_12  float32[32]  128B
  primals_13  float32[128x32]  16.0KB
  primals_14  float32[128]  512B
  primals_15  float32[32x128]  16.0KB
  primals_16  float32[32]  128B
Outputs (4.0KB total):
...
def run(inputs):
    native_layer_norm_1 = torch.ops.aten.native_layer_norm.default(inputs.add_1, [32], inputs.primals_11, inputs.primals_12, 1e-05)
    getitem_6 = operator.getitem(native_layer_norm_1, 0)
    getitem_7 = operator.getitem(native_layer_norm_1, 1)
    getitem_8 = operator.getitem(native_layer_norm_1, 2)
