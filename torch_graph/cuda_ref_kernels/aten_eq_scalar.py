"""Reference CUDA kernel for aten.eq_scalar (scalar comparison)."""
import torch

KERNEL_SRC = r"""
extern "C" __global__ void aten_eq_scalar(const float *in0, float *out0, unsigned int n) {
...
def expected(inputs):
    x = inputs[0]
    return [torch.ops.aten.eq.Scalar(inputs[0], 0.0).float()]
