"""Reference CUDA kernel for aten.fmod_scalar (scalar variant)."""
import torch

KERNEL_SRC = r"""
extern "C" __global__ void aten_fmod_scalar(const float *in0, float *out0, unsigned int n) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) { float x = in0[i]; out0[i] = fmodf(x, 2.0f); }
}
"""
...
def expected(inputs):
    x = inputs[0]
    return [torch.ops.aten.fmod.Scalar(inputs[0], 2.0)]
