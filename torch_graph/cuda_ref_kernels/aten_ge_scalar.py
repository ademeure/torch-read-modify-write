"""Reference CUDA kernel for aten.ge_scalar (scalar comparison)."""
import torch

KERNEL_SRC = r"""
extern "C" __global__ void aten_ge_scalar(const float *in0, float *out0, unsigned int n) {
...
def expected(inputs):
    x = inputs[0]
    return [torch.ops.aten.ge.Scalar(inputs[0], 0.0).float()]
