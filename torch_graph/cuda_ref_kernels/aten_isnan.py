...
extern "C" __global__ void aten_isnan(const float *in0, float *out0, unsigned int n) {
    ...
    if (i < n) { float x = in0[i]; out0[i] = (isnan(x) ? 1.0f : 0.0f); }
}
"""
...
def make_inputs(n=1024, seed=1):
    ...
    return [torch.tensor([1.0, float('nan'), 0.0, float('nan'), -1.0] * 200, device='cuda')]

def expected(inputs):
    x = inputs[0]
    return [torch.ops.aten.isnan.default(inputs[0]).float()]
