"""Reference CUDA kernel for aten.le_scalar (scalar comparison)."""
import torch

KERNEL_SRC = r"""
extern "C" __global__ void aten_le_scalar(const float *in0, float *out0, unsigned int n) {
...
def expected(inputs):
    x = inputs[0]
    return [torch.ops.aten.le.Scalar(inputs[0], 0.0).float()]
