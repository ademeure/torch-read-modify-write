"""Reference CUDA kernel for aten.logical_and."""
import torch

KERNEL_SRC = r"""
extern "C" __global__ void aten_logical_and(const float *in0, const float *in1, float *out0, unsigned int n) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) { float a = in0[i], b = in1[i]; out0[i] = ((a != 0.0f && b != 0.0f) ? 1.0f : 0.0f); }
}
"""
...
def expected(inputs):
    a, b = inputs
    return [torch.ops.aten.logical_and.default(a, b).float()]
