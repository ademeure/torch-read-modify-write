...
extern "C" __global__ void aten_tanh_backward(const float *grad, const float *saved, float *out0, unsigned int n) {
    ...
    if (i < n) { float g = grad[i], s = saved[i]; out0[i] = (g * (1.0f - s * s)); }
}
"""
...
def make_inputs(n=1024, seed=1):
    ...
    saved = torch.tanh(torch.randn(1024, device="cuda", generator=g))
    return [grad, saved]

def expected(inputs):
    grad, saved = inputs
    return [torch.ops.aten.tanh_backward.default(grad, saved)]
